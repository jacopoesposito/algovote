...
def create_app(client, private_key, approval_program, clear_program, global_schema, local_schema, globalInts, globalBytes, app_args):

    #setting sender as creator
    ...
    txn = transaction.ApplicationCreateTxn(sender,
        params,
        on_complete,
        approval_program,
        clear_program,
...
def opt_in(client, private_key, index):
    #declare sender 
    sender = account.address_from_private_key(private_key)
...
def delete_app(client, private_key, index):
    ...
    #display result
    transaction_response = client.pending_transaction_info(tx_id)
    print("Delete app-id:", transaction_response["txn"]["txn"]["apid"])
...
def close_out_app(client, private_key, index):
    ...
    #display results 
    transaction_response = client.pending_transaction_info(tx_id)
    print("Close out form app-id:", transaction_response["txn"]["txn"]["apid"])
...
def main():
    ...
    print(os.getcwd())
    with open("artifacts/vote_approval.teal", "r") as f:
        approval_program_teal = f.read()

    with open("artifacts/vote_clear_state.teal", "r") as f:
    ...
    opt_in(algod_client, user_private_key, app_id)
